...
#-----only push to database of loss and cobble step is done in chunk of 30 rows--------#
#---used for 1 sec granularity, Database Ingestion is done using Point (No Batching)---#
########################################################################################
...
bucket = "metaflow_test_for_cobble"
        ...
        try:
            for index, row in df.iterrows():
                for col_name, value in row.items():
                    ...
                    #if measurement_name == "CVR_L1_anomaly_score_cobble":
                        #print(current_time_str)
                    #iso8601_timestamp = current_time_str.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
                    data_point.time(current_time_str)
